...
local function CreateWindow(config)
    ...
    -- Đặt cạnh nhau góc phải-trên TopBar, cách nhau 26px. ZIndex mặc
    -- định (không set) vì TopBar/Root đã ClipsDescendants=true nên
    -- không cần nổi lên trên gì khác trong panel này.
    --------------------------------------------------------------
    ...
    CloseBtn.MouseButton1Click:Connect(function() DestroyWindow() end)
    ...
    -- Phóng to/thu nhỏ: đổi Ambient giữa kích thước gốc (winW x winH)
    -- và 1.4x kích thước đó, giữ tâm cố định (không dịch góc trên-trái
    -- khi phóng, để cảm giác giống "zoom từ giữa" chứ không "trồi ra
    -- một góc"). Toggle boolean đơn giản, không cần animate liên tục.
    local isMaximized = false
    ToggleMaximize = function()
        isMaximized = not isMaximized
        local w = isMaximized and math.floor(winW * 1.4) or winW
        local h = isMaximized and math.floor(winH * 1.4) or winH
        Tween(Ambient, {
            Size = UDim2.new(0, w, 0, h),
            Position = UDim2.new(0.5, -w / 2, 0.5, -h / 2),
        })
        MaxBtn.Text = isMaximized and "▢▢" or "▢"
    end
