...
local function CreateWindow(config)
    ...
    -- DisplayOrder mặc định của MỌI ScreenGui là 0 nếu không set tường
    -- minh -- ScreenGui chính này (chứa Root/Ambient/toàn bộ tab-control)
    -- trước đây KHÔNG set DisplayOrder, nên nó ở mức 0 và bất kỳ
    -- ScreenGui nào của GAME có DisplayOrder > 0 (rất phổ biến -- HUD,
    -- tutorial overlay, banner quảng cáo trong game đều hay tự set cao
    -- để nổi) sẽ vẽ ĐÈ LÊN TRÊN toàn bộ panel này, kể cả khi Root có
    -- ZIndex cao nội bộ -- ZIndex chỉ so sánh trong CÙNG một ScreenGui,
    -- không so sánh được xuyên ScreenGui khác nhau (đó là việc của
    -- DisplayOrder). Đây là nguyên nhân panel chính bị tutorial/nút
    -- game đè lên trong ảnh chụp thực tế.
    -- FIX: đặt DisplayOrder cao cho ScreenGui chính -- thấp hơn
    -- OverlayGui đúng 1 bậc (max-int - 1) để giữ đúng thứ tự NỘI BỘ:
    -- MenuButton (OverlayGui) vẫn phải nổi trên cả Root (ScreenGui
    -- chính), không được để hai lớp trùng DisplayOrder rồi phải cậy vào
    -- tie-break insertion-order mong manh.
    ...
    -- Self-healing top layer, cho CẢ HAI ScreenGui. DisplayOrder đặt lúc
    -- tạo chỉ đảm bảo "trên mọi thứ ĐANG tồn tại lúc đó" -- không nói gì
    -- về ScreenGui mà GAME parent vào PlayerGui một lúc sau. Hai kiểu vỡ
    -- cụ thể:
    --   1. Game UI cũng có DisplayOrder cao (rất phổ biến -- HUD,
    --      tutorial overlay, banner trong game thường tự set cao để nổi
    --      lên trên mọi thứ khác) -- nếu bằng hoặc cao hơn số của mình,
    --      Roblox vẽ theo thứ tự chèn (insertion order) khi trùng số,
    --      và ScreenGui chèn sau có thể đè lên cái chèn trước dù cùng
    --      DisplayOrder.
    --   2. Game re-parent hoặc tạo lại ScreenGui của nó sau khi mình đã
    --      tồn tại (rất hay gặp -- HUD dựng lại lúc respawn, đổi size
    --      class, chuyển trạng thái màn chơi). Instance mới đó là
    --      sibling được chèn SAU, cùng kiểu rủi ro tie-break như trên.
    -- Không thể poll mỗi frame (vi phạm hợp đồng no-Heartbeat/no-
    -- RenderStepped của file này) -- thay vào đó lắng nghe đúng sự kiện
    -- gây ra vấn đề: có gì đó mới trở thành ScreenGui-class con cháu của
    -- PlayerGui. DescendantAdded chỉ bắn khi cấu trúc thật sự đổi, không
    -- phải mỗi frame -- cùng triết lý event-driven như InputChanged ở
    -- chỗ khác trong file.
    -- CoreGui (chat, backpack mặc định, leaderboard) là lớp render RIÊNG
    -- BIỆT mà engine luôn vẽ trên mọi ScreenGui trong PlayerGui bất kể
    -- DisplayOrder -- không script client nào out-order được lớp đó;
    -- StarterGui:SetCoreGuiEnabled()/SetCore() là đòn bẩy duy nhất cho
    -- lớp đó, và file này không tự ý đụng vào.
    local function ReassertTop(gui)
        -- Đặt lại DisplayOrder = chính nó không giải quyết được tie
        -- (Roblox cần số CAO HƠN mới phá được tie bằng DisplayOrder) --
        -- nên khi thật sự đụng độ ở cùng mức, dùng insertion-order làm
        -- lưới an toàn: reparent ra rồi vào lại PlayerGui, biến nó
        -- thành ScreenGui được chèn gần nhất trong nhóm trùng
        -- DisplayOrder, thứ mà Roblox vẽ SAU CÙNG (= trên cùng) trong
        -- nhóm tie đó.
        gui.Parent = nil
    ...
    overlayWatcherConn = PlayerGui.DescendantAdded:Connect(function(inst)
        if inst == ScreenGui or inst == OverlayGui then return end -- chính mình tự bắn event này
        if not inst:IsA("ScreenGui") then return end
        -- OverlayGui luôn phải giữ vị trí trên cùng tuyệt đối (MenuButton
        -- không được để cái gì che, kể cả ScreenGui chính của chính
        -- mình) -- kiểm tra nó trước.
        if inst.DisplayOrder >= OverlayGui.DisplayOrder then
            ReassertTop(OverlayGui)
        end
        -- ScreenGui chính (Root/panel to) cũng cần tự-heal độc lập --
        -- đây là phần trước đây BỊ THIẾU, khiến panel bị tutorial/HUD
        -- của game đè lên dù OverlayGui vẫn nổi bình thường.
    ...
    -- v2.8: Fab tròn (ImageButton icon-only) đã bị XOÁ theo yêu cầu --
    -- chỉ còn MenuButton (pill có chữ) làm nút bật/tắt UI duy nhất.
    -- ToggleVisible/uiVisible/SetUIVisible vẫn giữ nguyên bên dưới vì
    -- MenuButton và bất kỳ keybind nào bind vào ToggleVisible vẫn cần
    -- chúng -- chỉ riêng phần dựng Instance của Fab bị gỡ.
