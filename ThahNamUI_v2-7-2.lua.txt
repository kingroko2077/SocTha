...
local function CreateWindow(config)
    ...
    -- Self-healing top layer. DisplayOrder = max-int at creation time only
    -- guarantees "on top of everything that exists right now" -- it says
    -- nothing about a ScreenGui the GAME parents into PlayerGui a frame
    -- later. Two concrete ways that breaks the naive version:
    --   1. Game UI also claims 2147483647 -- Roblox then falls back to
    --      insertion order among ties, and a later-inserted sibling can
    --      render on top of an earlier one with the same DisplayOrder.
    --   2. Game re-parents or recreates one of its own ScreenGuis after
    --      OverlayGui already exists (very common -- HUD elements get
    --      rebuilt on respawn, on a size class change, on a game-state
    --      transition). The recreated Instance is a NEW sibling inserted
    --      after OverlayGui, same tie-break exposure as above.
    -- Can't poll every frame for this (violates the no-Heartbeat/no-
    -- RenderStepped contract) -- instead, listen for the actual event
    -- that would ever cause the problem: something new becoming a
    -- ScreenGui-class descendant of PlayerGui. DescendantAdded is engine-
    -- debounced the same way InputChanged is elsewhere in this file --
    -- it only fires on the structural change itself, not per-frame.
    -- CoreGui (chat, default backpack, leaderboard) is a SEPARATE render
    -- layer the engine always draws above every PlayerGui ScreenGui
    -- regardless of DisplayOrder -- no client script can out-order it;
    -- StarterGui:SetCoreGuiEnabled()/SetCore() is the only lever for
    -- that layer, and this file doesn't touch it uninvited.
    local overlayWatcherConn
    overlayWatcherConn = PlayerGui.DescendantAdded:Connect(function(inst)
        if inst == OverlayGui then return end -- our own creation firing this same event
        if not inst:IsA("ScreenGui") then return end
        if inst.DisplayOrder >= OverlayGui.DisplayOrder then
            -- Reasserting the SAME max-int value on OverlayGui does nothing
            -- for a tie (Roblox needs a strictly higher number to break a
            -- tie by DisplayOrder alone) -- so on a genuine collision at
            -- max-int, fall back to sibling insertion order: reparent
            -- OverlayGui out and back into PlayerGui, making it the most
            -- recently inserted ScreenGui among any tied DisplayOrder,
            -- which Roblox draws last (= on top) within that tie.
            OverlayGui.Parent = nil
    ...
    -- FIX: Fab was anchored at Position = UDim2.new(1, -16, 0.5, 0) --
    -- vertical-center of the WHOLE ScreenGui. That comment's assumption
    -- ("this area is always empty") only holds when Root/Ambient is
    -- small relative to the screen (the 520x360 default). When Root is
    -- wide/tall enough to cover most of the viewport -- which it easily
    -- is on a phone screen, or after ToggleMaximize() -- mid-right of
    -- the ScreenGui lands INSIDE Root's bounds. Fab still renders on
    -- top (ZIndexBehavior.Sibling, ZIndex 60 vs Root's implicit 1, both
    -- direct children of ScreenGui -- Sibling mode compares them
    -- correctly), but BackgroundTransparency = 0.15 on the Glass tone
    -- sitting over Root's own near-identical Glass background made it
    -- visually disappear into the panel -- not a z-order bug, a
    -- contrast/placement one. Moved below Root instead: anchored to the
    -- bottom-right corner of the SCREEN, offset far enough down that it
    -- clears Root even at Root's default height, and clears it at
    -- Root's maximized height too since maximize leaves a 20px pad on
    -- every edge (see ToggleMaximize above).
    local Fab = New("ImageButton", {
        ...
        -- Bottom-right corner of the screen, 16px off each edge. Never
        -- inside Root's bounds regardless of Root's configured
        -- Width/Height or maximize state, since Root always keeps at
        -- least a 20px pad from every screen edge (Ambient's own
        -- position math, and ToggleMaximize's `pad = 20`).
    ...
    -- e.g. Banana Cat Hub / other mobile hubs). Distinct from the FAB:
    -- FAB is a small icon-only circle; this is a wider labeled pill
    -- meant to sit near the player's stat HUD, bottom-left by default.
    -- Both controls call the SAME SetUIVisible/uiVisible upvalues as
    -- the FAB above -- there is exactly one visibility state in the
    -- whole window, so FAB and MenuButton (and any keybind bound to
    -- ToggleVisible) can never drift out of sync with each other.
    --------------------------------------------------------------
    ...
    local MenuButton = New("TextButton", {
        ...
        -- Bottom-left, clear of the safe-area/notch strip the FAB
        -- comment already documents for the right edge. 16px inset off
        -- both edges so it doesn't collide with device home-indicator
        -- gesture zones on iOS.
